...
- **`.claude/hooks/docs-sync-reminder.sh`** — PostToolUse reminder, on edits to structural files the docs describe (`agents/*/{agent,main,prompts,__main__}.py`, `common/*.py`, `env.example`, `requirements.txt`, `.github/workflows/*.yml`), to keep the docs in lockstep: the root `README.md`, this `CLAUDE.md`, and (for edits inside an agent) that `agents/<name>/README.md`. It checks git and stays silent when the relevant doc has already been changed this session, so it only nags when the docs haven't been updated.
